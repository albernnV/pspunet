...
def data_load():
    ...
    train_size = int(0.6 * full_dataset_size)
    val_size = int(0.2 * full_dataset_size)
    test_size = int(0.2 * full_dataset_size)

    x_train = list_ds.take(train_size)
    y_train = list_ds_label.take(train_size)

    x_test = list_ds.skip(train_size)
    y_test = list_ds_label.skip(train_size)

    x_val = x_test.take(val_size)
    y_val = y_test.take(val_size)

    x_test = x_test.skip(test_size)
    y_test = y_test.skip(test_size)
    ...
    train_label_ds = y_train.map(get_label, num_parallel_calls=AUTOTUNE)

    train_dataset = tf.data.Dataset.zip((train_ds, train_label_ds))

    val_ds = x_val.map(process_path, num_parallel_calls=AUTOTUNE)
    val_label_ds_ = y_val.map(get_label, num_parallel_calls=AUTOTUNE)

    validation_dataset = tf.data.Dataset.zip((val_ds, val_label_ds_))

    test_ds = x_test.map(process_path, num_parallel_calls=AUTOTUNE)
    test_label_ds = y_test.map(get_label, num_parallel_calls=AUTOTUNE)

    test_dataset = tf.data.Dataset.zip((test_ds, test_label_ds))

    #list_ds_test = tf.data.Dataset.list_files("/content/drive/MyDrive/Exjobb/rgb_images(test_set)/*.png", shuffle=False)
    #list_ds_test_label = tf.data.Dataset.list_files("/content/drive/MyDrive/Exjobb/PSPUnet/semantic_annotations/semantic_annotations/gtLabels/*.png", shuffle=False)
